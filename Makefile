...
.PHONY: docker-start docker-stop docker-clean docker-rebuild docker-status docker-logs

docker-start: ## Build and start the application in Docker containers (detached mode)
	@if [ ! -f $(ENV_FILENAME) ]; then \
		$(MAKE) create-env; \
	fi
	$(DOCKER_COMPOSE) up --build -d

docker-stop: ## Stop and remove the application's Docker containers
...
docker-clean: ## Stop containers and remove all associated images, volumes, and networks
	$(DOCKER_COMPOSE) down --rmi all --volumes --remove-orphans

docker-rebuild: ## Rebuild the application's Docker images without using the cache
	@if [ ! -f $(ENV_FILENAME) ]; then \
		$(MAKE) create-env; \
	fi
	$(DOCKER_COMPOSE) up -d --force-recreate

docker-status: ## Display the status and port mappings of running containers
	@if [ -z "$$($(DOCKER_COMPOSE) ps --services --filter "status=running" 2>/dev/null)" ]; then \
...
docker-logs: ## Stream live logs from the running application container
	@if [ -z "$$($(DOCKER_COMPOSE) ps --services --filter "status=running" 2>/dev/null)" ]; then \
